...
class Trophies
{
	...
	static public function remAchieved(username:String, user_token:String, trophy_id:Int, ?onData:Bool->Void, ?onError:String->Void)
	{
		var url = Utils.formCall("trophies/remove-achieved/", ["game_id", "username", "user_token", "trophy_id"],
			[Std.string(Utils.game_id), username, user_token, Std.string(trophy_id)], 4);
		Utils.request(url, className, onData, onError);
	}
}